export const ELIGIBLE_IMAGE_TYPES = [
  "image/png",
  "image/jpeg",
  "image/gif",
  "image/webp",
];

export const ELIGIBLE_VIDEO_TYPES = ["video/mp4", "video/webm"];
export const MAX_FILE_SIZE = 100 * 1024 * 1024;
export const UPLOAD_FILE_EXPIRATION_TIME = 60 * 60;
export const READ_FILE_EXPIRATION_TIME = 60 * 60;
export const PROFILE_COVER_IMAGE_MAX_SIZE = 1024 * 1024;
export const SUBSCRIPTION_PLAN_IMAGE_MAX_SIZE = 1024 * 1024;
export const BLOG_POST_IMAGE_MAX_SIZE = 1024 * 1024;
export const STREAM_THUMBNAIL_IMAGE_MAX_SIZE = 2 * 1024 * 1024;
export const VIDEO_MAX_SIZE = 1024 * 1024 * 1024;
export const VIDEO_THUMBNAIL_IMAGE_MAX_SIZE = 2 * 1024 * 1024;
